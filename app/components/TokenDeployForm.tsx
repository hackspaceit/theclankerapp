          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="flex flex-col justify-end space-y-2">
              ...
              <input
                ...
                value={f.name}
                onChange={handle}
                className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded px-3 py-2 text-[#A25CD1] placeholder-[#666] focus:outline-none focus:border-[#A25CD1] transition-colors"
                required
              />
            </div>
            ...
            <div className="flex flex-col justify-end space-y-2">
              <div>
                <label className="block text-[#666]">Image (optional)</label>
            ...
            <div className="flex flex-col justify-end space-y-2">
              <label className="block text-[#666]">Token Fair</label>
              <select
                name="quote"
                value={f.quote}
                onChange={handle}
                className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded px-3 py-2 text-[#A25CD1] focus:outline-none focus:border-[#A25CD1] transition-colors appearance-none"
              >
            ...
            {/* Row 3 */}
            <div className="flex flex-col justify-end space-y-2">
              <label className="block text-[#666]">Initial marketcap</label>
              <input
                type="text"
                name="initialCap"
                value={f.initialCap}
                placeholder="1,5,6 or 10"
                onChange={handle}
                className="w-full bg-[#0a0a0a] border border-[#2a2a2a] rounded px-3 py-2 text-[#A25CD1] placeholder-[#666] focus:outline-none focus:border-[#A25CD1] transition-colors"
                required
              />
            ...
            <div className="flex flex-col justify-end space-y-2">
              <label className="block text-[#666]">Dev Buy</label>
              <input
                type="text"
                name="devBuyEth"
                value={f.devBuyEth}
            ...
            {/* Row 4 */}
            <div className="flex flex-col justify-end space-y-2">
              <label className="block text-[#666]">Vault Percentage (Optional)</label>
              <input
                type="number"
                name="vaultPct"
                value={f.vaultPct}
            ...
            <div className="flex flex-col justify-end space-y-2 col-span-2">
              <label className="block text-[#666]">Vault Duration (Optional)</label>
              <input
                type="number"
                name="vaultDays"
                value={f.vaultDays}
            ...
            {address ? (
              chainId === base.id ? (
                <button
                  ...
                  className="px-6 py-3 bg-[#0033FF] hover:bg-[#ffb84d] text-white font-medium rounded-lg transition-colors min-w-[200px] text-center cursor-pointer active:bg-[#cc7600] touch-manipulation disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <p className="text-[#0033FF]">Please switch to Base mainnet</p>
              )
            ) : (
          ...
          {phase === 'success' && tokenAddr && (
            <div className="mt-4 text-center">
              ...
              <div className="text-[#666] break-all text-sm mb-4">
                Token Address: <a href={`https://basescan.org/address/${tokenAddr}`} target="_blank" rel="noopener noreferrer" className="text-[#A25CD1] hover:underline">{tokenAddr}</a>
              </div>
              {/* Optional: Link to Clanker.world */}
              <div className="text-[#666] break-all text-sm mb-4">
                Manage on: <a href={`https://clanker.world/clanker/${tokenAddr}`} target="_blank" rel="noopener noreferrer" className="text-[#A25CD1] hover:underline">clanker.world</a>
              </div>
              <button
                onClick={reset}
